 ...
 * @author Piyush
 */
public class PerformMultipication {
    static int numberOfAdditions = 0;
    static int numberOfSubtractions = 0;
    static int numberOfMultiplications = 0;
    public void multiplication(int[][]matrix1,int[][]matrix2, int n1, int m1, int n2, int m2){
        int[][]multiplicationResult = new int[n1][m2];
        long startTime = System.nanoTime();
        
        /*Code for calculating the operations
        int i = 0;
        ...
        //for(int i = 0; i < n1; i++){
            ...
            while(j < m2){
                ...
                while(k < m1){
                    numberOfSubtractions++;
                    numberOfAdditions++;
                    //multiplicationResult[i][j] = (int) (multiplicationResult[i][j] + multiply(matrix1[i][k],matrix2[k][j]));
        ...
        for(int i = 0; i < n1; i++){
            for(int j = 0; j < m2; j++){
                for(int k = 0; k < m1; k++){
                    ...
                    numberOfAdditions++;
                    numberOfMultiplications++;
                    //double[][] newMatrix = multiply(matrix1,matrix2);
                }
            }
        }
        ...
        for(int l = 0; l < n1; l++){
            System.out.println("");
            for(int j = 0; j < m1; j++){
                System.out.print(multiplicationResult[l][j] + "  ");
            }
        }
        System.out.println("");
