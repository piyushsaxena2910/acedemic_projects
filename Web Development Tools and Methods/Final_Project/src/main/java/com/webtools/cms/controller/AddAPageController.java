...
@Controller
@RequestMapping("/admin/addAPage.htm")
public class AddAPageController {
	...
	public ModelAndView doSubmitAction(@ModelAttribute("page") Page page, BindingResult result, HttpServletRequest request) throws Exception {
		...
		if (result.hasErrors()) {
			ModelAndView mv = new ModelAndView("addAPage");
		     return mv;
		}
		...
		pageDAO.create(app,page.getPageName(),page.getArticle().getArticle(),page.getAside().getAside(),page.getFooter().getFooter(),page.getHeader().getHeader(),page.getNavigator().getNavigator(),page.getSection().getSection());
	...
	public ModelAndView initializeForm(@ModelAttribute("page") Page page, BindingResult result) {
		validator.validate(page, result);
		if (result.hasErrors()) {
			ModelAndView mv = new ModelAndView("addAPage", "userName", null);
		     return mv;
		}
